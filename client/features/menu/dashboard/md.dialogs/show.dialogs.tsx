...
export default function MenuDashboardShowDialogs() {
  ...
  console.log("Ini dari context", comityLoad);
  return (
    <Dialog
      open={isShow === "true" ? true : false}
      onOpenChange={() => router.replace("/dashboard")}
    >
      <DialogContent className="w-72 md:w-xl h-[650px] overflow-y-auto">
        ...
        <div>
          <h1 className="font-bold text-lg md:text-xl">Organization Vision</h1>
          <p className="px-2 py-1">{comityLoad?.vision.vision}</p>
        </div>

        <div>
          <h1 className="font-bold text-lg md:text-xl">Organization Mission</h1>
          <div className="overflow-y-auto h-80">
            {comityLoad?.mission.map((item: any, index: number) => (
              <p className="px-2 py-1 bg-gray-300 rounded-md my-2" key={index}>
                {" "}
